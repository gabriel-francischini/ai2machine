...
// Undefined behaviours led to random behaviours ;)
namespace Undefined{
mt19937 mt(1792 + time(NULL));
uniform_int_distribution<int> dist2(0,1);
}

//int counter_machine=0;
...
class Machine{
	...
	public:
		...
		Machine(int memory_space, int registers_amount){
			...
			//counter_machine += 1;
			//cout << counter_machine << ". Allocating machine with " << this->_memory_limit+1 << " chars and " << this->_registers_limit+1 << " ints.\n";
			this->memory = new char[this->_memory_limit+1];
			this->registers = new int[this->_registers_limit+1];
			this->clearMemory();
		}
		
		Machine(Machine &copied){
			this->_memory_limit = copied.memory_limit();
			this->_registers_limit = copied.registers_limit();
			//counter_machine += 1;
			//cout << counter_machine << ". Allocating machine copy with " << this->_memory_limit+1 << " chars and " << this->_registers_limit+1 << " ints.\n";
			this->memory = new char[this->_memory_limit+1];
			this->registers = new int[this->_registers_limit+1];
			this->clearMemory();	
		}

		Machine& operator=(const Machine& assigned){
            ...
            if (this != &assigned){
                ...
                //cout << counter_machine << ". Deallocating machine assigment with " << this->_memory_limit+1 << " chars and " << this->_registers_limit+1 << " ints.\n";
				delete[] this->memory;
				delete[] this->registers;
                this->_memory_limit = assigned.memory_limit();
                this->_registers_limit = assigned.registers_limit();
                //cout << counter_machine << ". Allocating machine assigment with " << this->_memory_limit+1 << " chars and " << this->_registers_limit+1 << " ints.\n";
				this->memory = new char[this->_memory_limit+1];
				this->registers = new int[this->_registers_limit+1];

                // Copy assigned's memory and registers
                for(int i=0; i<= this->_memory_limit;i++)
                    this->memory[i] = assigned.memory[i];

                for(int i=0; i<=this->_registers_limit;i++)
                	this->registers[i] = assigned.registers[i];
            }
		...
		~Machine(){
			//counter_machine -= 1;
			//cout << counter_machine << ". Deallocating machine with " << this->_memory_limit+1 << " chars and " << this->_registers_limit+1 << " ints.\n";
			delete[] this->memory;
			delete[] this->registers;
		}
};
...
void Machine::printMemory(){
	...
	cout.write(&this->memory[0], this->_memory_limit+1);
}
...
int Machine::execute(unsigned char instruction, char *value1, char *value2,
	...
	switch(instruction){
		...
		case MUL_REG_VALUE:
		{
			...
			if(Undefined::dist2(Undefined::mt))
				SETFLAG(*flags, AUXILIARY_CARRY_FLAG);
			else
				UNSETFLAG(*flags, AUXILIARY_CARRY_FLAG);
...
// Loads (i.e. copy) a array's memory inside a machine's memory
void Machine::loadOnMemory(char *memory, int length){
...
void Machine::clearMemory(){
	for(int i=0; i <= this->_memory_limit;i++)
		this->memory[i]= 0;//Undefined::distchar(Undefined::mt);
	for(int i=0; i < this->_registers_limit+1; i++)
		registers[i] = 0;//Undefined::distchar(Undefined::mt);
	for(int i=0; i < 3; i++)
		registers[i] = 0;
}
