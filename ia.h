...
int machine::getTicket(){
	...
	for(int i=-1;i<2; i++){
		...
		int stack_position = (int) this->memory_limit-( (*sp) + i );	
...
int machine::execute(unsigned char instruction, int *value1, int *value2){
	...
	int value_1 = *value1, value_2 = *value2;
...
void gene::reproduce(gene *father, gene *mother){
	if((rand()%100)< CROSSOVER_RATE){
		int crossover_place = rand()%(this->memory_limit+1);
		gene *copy=father;
		for(int i=0;i <= this->memory_limit; i++){
